...
def supadd():
    ...
    try:
        params = (str(mid), nam, bran, purpos, str(expir), dosag, str(pric), typ, str(quantit), session['type'], )
        print(params)
        print(query)
    ...
    try:
        if it:
            ...
            if it1[0]==session['role']:
                ...
                connection.commit()
                msg = "Препарат обновлен!!"
                category ="success"
                return msg, category
            else:
                msg = "Препарат вами не поставляется!!"
                category = "warning"
                return msg, category
        else:
            msg = "Препарата не существует!!"
            category="danger"
            return msg, category
    except mysql.connector.Error as err:
        print(err)
